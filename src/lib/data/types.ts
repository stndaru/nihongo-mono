...
export interface ExampleSentence {
  ...
  en: string
  /**
   * Furigana for `ja` in compact bracket form — 漢字[かんじ] runs inline
   * with the plain text (parse with parseFurigana). Generated by kuromoji
   * at build time (scripts/lib/reading.ts); absent for kana-only sentences.
   * A string instead of segment objects: measured 5–10× smaller on the wire.
   */
  f?: string
}
...
/** One cross-link to another grammar point, with an optional contrast note. */
export interface GrammarRelation {
  /** slug of another grammar point in this dataset */
  slug: string
  /** one-line "vs" note ("ないで attaches to verbs; なくて states a cause") */
  note?: string
}

/**
 * One JLPT grammar point (src/data/grammar, packed to
 * public/data/jlpt/grammar-n{level}.json.gz). All prose is original content
 * — the source sites contributed only the inventory (which points exist,
 * their level, a one-line meaning hint); see docs/data-pipeline.md.
 */
export interface GrammarEntry {
  /**
   * Stable, level-free, lowercase-romaji id — the route param and the key
   * relations point at. NEVER renamed once cross-referenced; a re-leveled
   * point moves files but keeps its slug.
   */
  slug: string
  /** Display pattern in Japanese script ("ないで", "〜たことがある"). */
  title: string
  /** Kana reading of `title` (search key; equals title when already kana). */
  kana: string
  /** Hepburn romaji of `kana`, spaced ("hou ga ii"), for latin search. */
  romaji: string
  /** Short English gloss for the table row ("without doing"). */
  meaning: string
  jlpt: JlptLevel
  /** 2–4 sentence original explanation (plain prose, no markup). */
  summary: string
  /**
   * Formation lines, one per accepted formation. Convention (rendered by
   * GrammarStructure): tokens joined with ＋, form annotations in full-width
   * ［…］ after the part name, alternatives with ／ inside one token —
   * "Verb［ない form］＋ で", "な-adjective ＋ で／であって".
   */
  structure: string[]
  /** Caution / common-mistake notes, one bullet each. May be empty. */
  pitfalls: string[]
  /** Exactly 2, original sentences; `f` is generated by the build, never authored. */
  examples: ExampleSentence[]
  synonyms: GrammarRelation[]
  antonyms: GrammarRelation[]
  /** Same-family / easily-confused points that are neither of the above. */
  related: GrammarRelation[]
  /** Provenance of the inventory row — the curated-union audit trail. */
  sources: ('jlptsensei' | 'bunpro')[]
  /** Present when sources disagreed or review re-leveled ("bunpro lists N4"). */
  levelNote?: string
}
