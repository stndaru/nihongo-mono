  ...
  return (
    <div className="mx-auto max-w-xl space-y-5" {...quizDisplayAttrs(display)}>
      ...
      <div className="rounded-lg border p-5 text-center">
        {question.kind === 'recall' || question.kind === 'word' ? (
      ...
      {state.phase === 'question' ? (
        question.kind === 'meaning' ? (
          ...
          <WordChoices
            key={state.index}
            choices={question.wordChoices!}
            onSelect={(choice) =>
              dispatch({ type: 'answer', given: choice.kanji, correct: choice.id === question.word.id })
            }
          />
        ) : (
          <KanaInput
...
/** EN → JA: the meaning is on screen, the options are Japanese words. */
function WordChoices({
  choices,
  onSelect,
}: {
  choices: VocabEntry[]
  onSelect: (choice: VocabEntry) => void
}) {
    ...
    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
      {choices.map((choice, i) => (
        ...
        >
          ...
          {/* readings don't spoil an EN→JA match; the session-wide
              Furigana toggle ([data-quiz-ruby]) governs them anyway */}
          <Furigana segments={choice.furigana} className="text-xl leading-tight" />
        </button>
      ))}
    </div>
  )
