...
- **No backend**: static hosting only (the owner deploys with
  ...
  **Offline access** download (decisions 72/74) caches the base app via a
  service worker; Paddle/Tesseract OCR is a separate optional pack.
...
| `/cheatsheet/counters` | Counters cheatsheet: "How Counters Work" cards + number-systems trivia accordion, the universal つ series, must-know and common counter tables whose rows expand into generated counting sequences (1–100, from `src/lib/counters.ts` sound-change rules), sound-change box, irregular dates — content based on Tofugu's counters guide + list (credited on the page) |
...
| `/parser` | Sentence parser: paste kana/kanji text with numbers and ASCII/full-width Latin names (≤120 chars; auto-growing, drag-resizable input) for a word-by-word breakdown. Mixed input is preserved for whole-sentence translation and Microsoft IME composition; Latin stays visible in the breakdown as untagged gray context with an explanatory tooltip, but neither engine analyzes it and it never enters Words Found. A direction toggle adds an independent English → Japanese tab (≤200 chars of English → machine-translated JP → same breakdown; separate ?en=/?dir= state, standing accuracy warning). Greedy matching by default; confirm-gated "Smart Parsing" opt-in (~17 MB kuromoji) adds furigana, POS-colored underlines, Beyond-tier links, a reading fallback for variant spellings, and dictionary-validated compound merging. Confirm-gated Scan Image uses on-device PaddleOCR (~26.8 MB) for printed, vertical, and handwritten Japanese/English, with automatic Tesseract fallback and a low-confidence review flow. An async English translation section loads alongside the breakdown (Google → MyMemory → external-link fallback) |
...
| `/settings` | Theme, font toggles, font sizes (global / kanji & kana / furigana, Default–Largest each), progress export/import/reset, the **Cloud sync** section, the ~60 MB base **Offline access** section, and a separate ~34 MB image-scanning offline pack with independent remove controls |
